...
We're using the `await` here to wait for the connection between Prisma and our Neon database to be established.  

Later on, when we'll see our methods inside our `Employee` service, we won't see any `await`, only `async`.  
That is because **Prisma**'s query methods return **promises**.  

Our **service methods** will simply pass these **promises** along to the **controllers**, and NestJS will handle 
resolving them before sending the response.  

## The Employees REST API

With our database module and service now created, we are ready to start building our Employee REST API.  
...
- an `employee.entity.ts` file, which is a class meant to represent our data model

It will also update the app.module.  

**IMPORTANT note**:  
We're not going to use the **entity** file because we'll be using the **data model** that we've defined 
in our `schema.prisma` file.  
And we won't use the **DTOs** either because we will use the **types** from **Prisma**.  
So we can delete the `dto` and `entities` folders.
...
We will rely on the **types** generated by `npx prisma generate` (like the `Employee` type from `@prisma/client`).
